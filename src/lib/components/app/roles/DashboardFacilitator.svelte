...
<div class="animate-in fade-in flex flex-col gap-10 duration-1000 md:gap-14">
		...
		<div class="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
			<div class="space-y-4">
				<div class="flex items-center gap-3">
					<div
						class="rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-[10px] font-black tracking-widest text-indigo-700 uppercase shadow-sm dark:border-indigo-900/30 dark:bg-indigo-900/20"
					>
						Facilitator Hub
					</div>
					<div
						class="h-1.5 w-1.5 animate-pulse rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]"
					></div>
				...
				>
					Cohort <span
						class="bg-linear-to-r from-indigo-600 via-blue-500 to-purple-600 bg-clip-text text-transparent"
						>Control</span
					>
				</h1>
				<p
				...
				>
					Managing student experience and cohort success. View active batches, monitor engagement,
					and ensure consistent learning velocity across your groups.
				</p>
			</div>
			...
			<div class="flex items-center gap-3">
				<a href="/app/facilitator/cohorts">
					<Button
						variant="primary"
						class="h-12 px-8 font-black tracking-widest uppercase shadow-xl shadow-indigo-500/10 transition-transform hover:scale-105 active:scale-95"
					>
						Manage All Batches
					</Button>
				</a>
	...
	<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
		...
		<div
			class={`group relative p-8 ${RADIUS.card} ${COLOR.card} border ${COLOR.cardBorder} ${ELEVATION.card} ${TRANSITION.all} border-l-4 border-l-indigo-600 hover:-translate-y-1`}
		>
			<div class="space-y-1">
				<span class="text-[10px] font-black tracking-widest text-zinc-400 uppercase"
					>Total Students</span
				>
				<p
					class="text-5xl leading-tight font-black tracking-tighter text-indigo-600 dark:text-indigo-400"
				>
			...
			>
				...
				<div class="space-y-3">
					<a
						...
						class={`flex items-center justify-between p-4 ${RADIUS.button} border ${COLOR.cardBorder} transition-all hover:bg-zinc-50 dark:hover:bg-zinc-800`}
					>
						<div class="flex items-center gap-3">
	...
	<PageSection>
		...
		<div
			class={`overflow-hidden ${RADIUS.card} border ${COLOR.cardBorder} ${COLOR.card} shadow-sm`}
		>
			{#if rows.length > 0}
				<Table {columns} {rows} />
			{:else}
				<div class="p-12 text-center text-zinc-400">
					<p class="text-sm font-medium tracking-widest uppercase">No active batches assigned</p>
				</div>
			{/if}
		</div>
	</PageSection>
