...
>
	...
	>
		...
		<div class="flex items-center justify-between py-2">
			<label class="flex items-center gap-2 cursor-pointer group">
				<div class="relative flex items-center">
					...
					/>
					<svg class="absolute h-3.5 w-3.5 text-white opacity-0 transition-opacity peer-checked:opacity-100 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">
						<polyline points="20 6 9 17 4 12"></polyline>
					</svg>
				</div>
				<span class={`${TEXT.small} ${COLOR.textSecondary} group-hover:text-zinc-900 dark:group-hover:text-white transition-colors`}>Ingat saya</span>
			</label>
			
			<a href="/auth/forgot" class={`${TEXT.small} font-bold text-blue-600 hover:text-blue-700 transition-colors`}>
				Lupa password?
			</a>
		</div>
		...
		<p class={`text-center pt-4 ${TEXT.small} ${COLOR.textSecondary}`}>
			Belum punya akun? 
			<a href="/auth/signup" class="font-bold text-blue-600 hover:text-blue-700 transition-colors ml-1">
				Daftar di sini
			</a>
		</p>
	</form>
