...
<PageWrapper>
	...
	<div class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
		<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border ${COLOR.cardBorder} p-5 text-center`}>
			...
			<p class={`text-xs font-bold uppercase tracking-widest ${COLOR.textMuted}`}>Enrolled</p>
		</div>
		<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border ${COLOR.cardBorder} p-5 text-center`}>
		...
		<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border ${COLOR.cardBorder} p-5 text-center`}>
			...
			<p class={`text-xs font-bold uppercase tracking-widest ${COLOR.textMuted}`}>Lessons Done</p>
		</div>
		<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border ${COLOR.cardBorder} p-5 text-center`}>
			<p class="text-4xl font-black text-amber-500">{data.stats.avgScore || '-'}</p>
			<p class={`text-xs font-bold uppercase tracking-widest ${COLOR.textMuted}`}>Avg Score</p>
		</div>
	</div>
	...
	<div class="mb-8">
		...
		{#if data.enrollments.length === 0}
			<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border ${COLOR.cardBorder} p-8 text-center`}>
				<p class={`${COLOR.textSecondary}`}>You haven't enrolled in any courses yet.</p>
				<a href="/app/courses" class="mt-4 inline-block text-blue-600 hover:underline">Browse Courses</a>
			</div>
		{:else}
			<div class="space-y-4">
						...
						<div class="flex items-start gap-4">
							<div class="h-16 w-24 shrink-0 overflow-hidden rounded-lg bg-gray-100">
								{#if enrollment.course.thumbnailUrl}
									<img src={enrollment.course.thumbnailUrl} alt="" class="h-full w-full object-cover" />
								{:else}
							...
							<div class="flex-1 min-w-0">
								<div class="flex items-center gap-2 mb-1">
									<h3 class={`font-bold ${COLOR.textPrimary} truncate`}>{enrollment.course.title}</h3>
									{#if enrollment.track && trackLabels[enrollment.track]}
										<span class={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${trackLabels[enrollment.track].color}`}>
								...
								<div class="mb-2">
									<div class="flex items-center justify-between mb-1">
										...
										<span class={`text-xs font-bold ${enrollment.progressPercent >= 100 ? 'text-green-600' : 'text-blue-600'}`}>
											{enrollment.progressPercent}%
										</span>
									</div>
									<div class="h-2 w-full overflow-hidden rounded-full bg-gray-100">
										<div
											class="h-full rounded-full transition-all {enrollment.progressPercent >= 100 ? 'bg-green-500' : 'bg-blue-500'}"
								...
								<div class="flex items-center gap-3">
									...
									{#if enrollment.progressPercent > 0 && enrollment.progressPercent < 100}
										<a href="/app/courses/{enrollment.course.id}/learn" class="text-xs text-blue-600 hover:underline">
											Continue →
										</a>
									{/if}
								</div>
				...
				<table class="w-full text-left">
					<thead>
						<tr class="border-b border-gray-100 bg-gray-50/70">
							<th class={`px-5 py-3 ${TEXT.small} font-black uppercase tracking-widest ${COLOR.textMuted}`}>Course</th>
							<th class={`px-5 py-3 ${TEXT.small} font-black uppercase tracking-widest ${COLOR.textMuted}`}>Type</th>
							<th class={`px-5 py-3 ${TEXT.small} font-black uppercase tracking-widest ${COLOR.textMuted}`}>Score</th>
							<th class={`px-5 py-3 ${TEXT.small} font-black uppercase tracking-widest ${COLOR.textMuted}`}>Date</th>
						</tr>
					</thead>
					<tbody class="divide-y divide-gray-50">
						{#each data.submissions as sub}
							<tr>
								...
								<td class="px-5 py-3">
									{#if sub.score !== null}
										<span class={`font-bold ${sub.score >= 70 ? 'text-green-600' : 'text-amber-600'}`}>
											{sub.score}/100
										</span>
									{:else}
		...
		<div>
			...
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
				{#each data.certificates as cert}
					<div class={`${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border-2 border-amber-200 bg-amber-50/50 p-5`}>
						...
						<p class="text-xs font-mono text-gray-500 mb-3">{cert.serial}</p>
						<a href="/app/certificates/{cert.id}" class={`text-sm font-bold text-blue-600 hover:underline`}>
							View Certificate →
						</a>
					</div>
				{/each}
