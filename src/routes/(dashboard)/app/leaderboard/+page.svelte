...
<PageWrapper>
	...
	{#if data.userXP}
		<div
			class={`mb-8 ${RADIUS.card} ${COLOR.card} ${ELEVATION.base} border-2 border-amber-200 bg-amber-50/50 p-6`}
		>
			<div class="flex items-center gap-6">
				<div class="text-center">
					<p class="text-4xl font-black text-amber-500">#{data.userRank || '-'}</p>
					<p class={`text-xs font-bold tracking-widest uppercase ${COLOR.textMuted}`}>Your Rank</p>
				</div>
				<div class="flex-1">
				...
				<div class="mt-4 border-t border-amber-200 pt-4">
					...
					<div class="flex flex-wrap gap-2">
						{#each data.userBadges as badge}
							<div
								class="flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-sm font-bold text-amber-700"
							>
	...
	{#if data.leaderboard.length >= 3}
		<div class="mb-8 flex items-end justify-center gap-4">
			...
			<div class="text-center">
				<div
					class="mx-auto mb-2 flex h-16 w-16 items-center justify-center rounded-full bg-gray-200 text-2xl font-bold"
				>
					🥈
				</div>
				<p class={`font-bold ${COLOR.textPrimary}`}>
					{data.leaderboard[1].fullName || data.leaderboard[1].username}
				</p>
				<p class="text-sm text-gray-500">{data.leaderboard[1].points} XP</p>
				<div class="mt-2 flex h-24 w-20 items-end justify-center rounded-t-lg bg-gray-300 pb-2">
			...
			<div class="-mt-4 text-center">
				<div
					class="mx-auto mb-2 flex h-20 w-20 items-center justify-center rounded-full bg-amber-100 text-3xl font-bold"
				>
					🥇
				</div>
				<p class={`font-bold ${COLOR.textPrimary}`}>
				...
				<div
					class="mt-2 flex h-32 w-24 items-end justify-center rounded-t-lg bg-gradient-to-t from-amber-300 to-amber-200 pb-2"
				>
			...
			<div class="text-center">
				<div
					class="mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-orange-100 text-xl font-bold"
				>
					🥉
				</div>
				<p class={`font-bold ${COLOR.textPrimary}`}>
				...
				<div class="mt-2 flex h-16 w-16 items-end justify-center rounded-t-lg bg-orange-200 pb-2">
			...
			<tbody class="divide-y divide-gray-50">
				{#each data.leaderboard as entry}
					<tr class={entry.userId === data.userXP?.userId ? 'bg-blue-50/50' : 'hover:bg-gray-50'}>
						<td class="px-5 py-3">
							<span
								class={`text-lg font-black ${entry.rank <= 3 ? 'text-amber-500' : 'text-gray-400'}`}
							>
						...
						<td class="px-5 py-3">
							<span
								class="rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-bold text-purple-700"
							>
								Lv. {entry.level}
							</span>
						</td>
