...
export const load: PageServerLoad = async (event) => {
	...
	if (!membership || !['admin', 'facilitator', 'mentor', 'owner'].includes(membership.role)) {
	...
	// Get cohort details, verifying it belongs to the organization
	const cohortResult = await db
		...
		.where(and(eq(schema.cohort.id, cohortId), eq(schema.course.orgId, orgId)))
		.limit(1);

	if (!cohortResult[0]) throw error(404, 'Cohort tidak ditemukan dalam organisasi ini');
	...
	// Build student progress data - using bulk queries would be better, but keeping logic for now
	...
	return { cohort, students, membership };
};
...
export const actions: Actions = {
	sendReminder: async (event) => {
		...
		// Verify target user is in the cohort AND cohort is in the organization
		...
		if (targetCheck.length === 0) return fail(403, { error: 'Target user not found in this cohort context' });
