...
export const load = async ({ locals, url }) => {
	...
	const [membership] = await db
		...
		.where(
			and(
				...
				or(
					eq(organizationMember.role, 'facilitator'),
	...
	if (membership) {
		const newPath = url.pathname.replace('/app/facilitator', `/app/organizations/${membership.orgId}/facilitator`);
		// If the root was requested, go to cohorts as default
		const finalPath = newPath === `/app/organizations/${membership.orgId}/facilitator` 
			? `${newPath}/cohorts` 
			: newPath;
