...
interface OptimizedImageProps {
	...
	sources?: Array<{
		// For picture element (art direction)
		srcset: string;
		media?: string;
		type?: string;
	}>;
...
```svelte
<OptimizedImage src="/images/avatar.jpg" alt="User avatar" width={60} height={60} loading="lazy" />
```
...
```svelte
<SectionNavigation showProgress={true} showBackToTop={true} scrollOffset={70} />
```
